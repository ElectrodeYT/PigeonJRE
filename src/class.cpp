...
Class::Class(std::string path) {
  ...
  if (!file.good()) { throw std::invalid_argument("Input error"); }
  // Get file size
...
void Class::loadData() {
  ...
  cafebabe[2] = readByte();
  cafebabe[3] = readByte();
  if (cafebabe[0] != 0xCA || cafebabe[1] != 0xFE || cafebabe[2] != 0xBA || cafebabe[3] != 0xBE) { throw std::invalid_argument("File not a class file"); }
  ...
  if (file_major_version >= 56) {
    if (file_minor_version != 0 && file_minor_version != 65535) { throw std::invalid_argument("File is corrupted; invalid minor version"); }
  }
  ...
  for (size_t i = 1; i < cp_size; i++) {
	...
	for (size_t y = 0; y < constant_pool.size(); y++) {
	  ...
	  if (constant_pool[y].tag == CPTag::UTF8) { std::cout << "\t\t" << constant_pool[y].string << "\n"; }
	}
...
std::vector<Method> Class::readMethods() {
  ...
  for (size_t i = 0; i < amount; i++) {
    ...
    for (size_t y = 0; y < m.attributes.size(); y++) {
      ...
      code.max_stack = be16toh(*((uint16_t*)(attr_info)));
      attr_info += 2;
      code.max_locals = be16toh(*((uint16_t*)(attr_info)));
      attr_info += 2;
      uint32_t code_size = be32toh(*((uint32_t*)(attr_info)));
      attr_info += 4;
      code.code.resize(code_size);

      // Copy the code itself
      memcpy(code.code.data(), attr_info, code_size);
      attr_info += code_size;
      // Read the exception tables
      uint16_t exception_table_length = be16toh(*((uint16_t*)(attr_info)));
      attr_info += 2;
      for (size_t x = 0; x < exception_table_length; x++) {
	CodeAttribute::ExceptionTable exc;
	exc.start_pc = be16toh(*((uint16_t*)(attr_info)));
	attr_info += 2;
	exc.end_pc = be16toh(*((uint16_t*)(attr_info)));
	attr_info += 2;
	exc.handler_pc = be16toh(*((uint16_t*)(attr_info)));
	attr_info += 2;
	exc.catch_type = be16toh(*((uint16_t*)(attr_info)));
	attr_info += 2;
	code.exception_tables.push_back(exc);
      }
      // Read the attributes
      uint16_t attributes_length = be16toh(*((uint16_t*)(attr_info)));
      attr_info += 2;
      for (size_t x = 0; x < attributes_length; x++) {
	Attribute a;
	a.name = readAndVerifyCPEntry(be16toh(*((uint16_t*)(attr_info))), CPTag::UTF8).string;
	attr_info += 2;
	uint32_t a_size = be32toh(*((uint32_t*)(attr_info)));
	attr_info += 4;
